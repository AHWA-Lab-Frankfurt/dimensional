...
dimension_relevance(decor.graph, actors = c(2:7,15,18:19), dimensions = name, weighted = TRUE)
...
dimheat <- decor.graph %>% 
  dimension_relevance(dimensions = name, weighted = TRUE) %>% 
 # mutate(Var1 = factor(row.names(.), levels=row.names(.))) %>% 
  select(-id, -neighbor) %>% 
  filter(label != "mean") %>% 
  gather(key = "decor", value = "dimension_relevance", -label, na.rm = TRUE) 
...
dimension.node.a <- dimension_correlation_node(decor.graph, actors = c(2:7,15,18:19), dimensions = name, weight = TRUE)


#plotting a heatmap for the correlation matrix
dimension.node.a[lower.tri(dimension.node.a, diag = TRUE)] <- NA
cor_tri <- as.data.frame(dimension.node.a) %>% 
  mutate(Var1 = factor(row.names(.), levels=row.names(.))) %>% 
  gather(key = Var2, value = value, -Var1, na.rm = TRUE, factor_key = TRUE) 
